...
// VARIAVEIS
int LINHA, COLUNA, PASSOS, THREADS, SEED, LIMIAR; // HEADER; threads eh util nessa versão
...
int main(int argc, char* argv[]) {
        ...
        while(true) {
            ...
            for(int i = 0; i < LINHA; i++) {
                for(int j = 0; j < COLUNA; j++) {
                    ...
                    if (!(estado_atual[indice].tempo_ativacao == tempo_atual && estado_atual[indice].es == CONTENCAO)) {
                        prox_estado[indice] = estado_atual[indice];
                    }
    ...
    for(long long i = 0; i < LINHA*COLUNA; i++) { // Vou mudar todos os for para isso, que loucura boa/ tem que verificar se eh melhor pro omp, mas acho que sim
                                                  // Acho que nao eh melhor nao, pq precisa ficar recuperando i e j la em cima slc
